Shader "Custom/GroundCompute"
{
	Properties
	{
		...
		_LightRef("Light Reflection Value", Float) = 1
		_GrassLenghtRef("The grass lenght value of Reflect", Float) = 1
		_Radius("Radius of Reflction", Float) = 1
		_RefColor("ReflectionColor " , Color) = (1,1,1,1)
		_TopTint("TopTint " , Color) = (1,1,1,1)
		_BottomTint("Bitton Refliction " , Color) = (1,1,1,1)
	}
	...
	SubShader
	{
		...
		Pass
		{
            ...
            return 0;
				...
				//float4(ShadeSH9(float4(0,0,1,1)),0);
				...
				// extra point lights support__________________________
				float3 extraLights;
				...
				// Light Reflection____________________________________

				//the normal of the grass
				float3 Normal = i.normalWS;
				//some how the light reflect the opisset on the z so i added nevative
				float3 LightSource = float3(_MainLightPosition.x, _MainLightPosition.y, -_MainLightPosition.z);
				//the reflected vetor from the light sourse 
				float3 LightReflect = normalize(reflect(-LightSource, Normal));
				// the view vector to the single grass 
				float3 ViewPosition = normalize(i.positionWS - _WorldSpaceCameraPos);
				//the defranse between the perfect reflect and the view angle 
				float LightValue = -max(0, 1 - dot(ViewPosition, LightReflect) - _Radius);
				//add the light on top of th grass 
				float verticalLightFade = saturate(i.uv.y + LightValue);
				// lerp between the value color and the grass main color 
				baseColor = lerp(baseColor, _MainLightColor + _RefColor + (LightValue * _LightRef),
				                 saturate(verticalLightFade + _GrassLenghtRef));

				// get the floor map____________________________________
